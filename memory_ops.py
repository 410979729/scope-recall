...
def merge_memories(provider: Any, target_id: str, source_ids: list[str], content: str | None = None, target: str | None = None) -> dict[str, Any]:
    ...
    updated, summary, updated_at = update_memory(provider, target_id, merged, target or str(target_row["target"]))
    if not updated:
...
def export_memories(provider: Any, *, fmt: str = "jsonl", scope_only: bool = True) -> dict[str, Any]:
    conn = provider._require_conn()
    where = "WHERE scope_id = ?" if scope_only else ""
    params: tuple[Any, ...] = (provider._scope_id,) if scope_only else ()
    with provider._lock:
        rows = conn.execute(
            f"""
...
def stats_payload(provider: Any) -> dict[str, Any]:
    ...
    with provider._lock:
        ...
        scoped = conn.execute("SELECT COUNT(*) FROM memories WHERE scope_id = ?", (provider._scope_id,)).fetchone()[0]
    vector_path = ""
    vector_table = ""
